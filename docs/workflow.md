...
- 缺少 `.vibespec/state.json`：skill 停止阶段推进，并引导用户创建或恢复 repo-local protocol；
- 状态文件存在但无效或损坏：validator 报错，并要求用户选择修复、重新初始化 protocol 或人工恢复；
- 状态文件为空对象 `{}`：视为未初始化；
- 配置文件 `.vibespec/config.json` 解析或 schema 校验失败：终止阶段推进并输出校验错误。

#### 状态与产物一致性

文件系统是产物存在性的权威来源。每次加载状态时，VibeSpec 会验证 `.vibespec/state.json` 中记录的产物路径在磁盘上是否存在。若产物文件不存在，对应阶段标记为未完成（无论状态文件记录如何）。若产物文件存在但状态文件标记为未完成，则修正状态文件以反映实际进度。
